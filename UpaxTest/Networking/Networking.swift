...
class Networking {
    ...
    // MARK: - Handlers
    func fetchService(service: RequestProtocol, completion: @escaping ServiceModelResponse) {
        ...
        session.dataTask(with: request) { (data, response, error) in
            ...
            completion(ParserHelper.parseObject(of: ServiceResponseModel?.self, data: json))
        }.resume()
    }
